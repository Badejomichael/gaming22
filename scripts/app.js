      ...
      for (const category of categories) {
          ...
          if (data.results.length > 0) {
              ...
              categoryData.push({
                  ...
                  link: `games.html?category=${category.slug}`
              });
          }
      }
      ...
      const categoryHTML = `
          <div class="slide">
            <div class="card" style="width: 16rem">
              ...
              <a href="${category.link}" class="btn btn-outline-light">View Games</a>
            </div>
          </div>
      `;
...
function displayGameNews() {
  ...
  gameNews.forEach((game) => {
    ...
    `
    <div class="card">
      <img src="${game.image}" class="card-img-top" alt="..." height="150px"/>
      <div class="card-body">
        <h5 class="card-title">${game.name}</h5>
        <p class="card-text">
        ${game.briefNews}
      ...
      <buttton class="btn btn-outline-light js-read-more-news-btn" data-game-id="${game.id}">🔗 Read More</button>
    </div>
    `
  });

  document.querySelector('.js-games-news-row').innerHTML = gameNewsHTML;
